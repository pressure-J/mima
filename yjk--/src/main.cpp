...
//   approx_cor batch-all <max_r> [--beam N] [--output dir] [--threads N]
//   approx_cor batch-all-parallel <max_r> [--beam N] [--output dir] [--threads N]
...
// 全局单例定义
LatTable g_lat;
PrecomputedTransitions g_precomp;
}  // namespace hs
...
void print_usage() {
  ...
  approx_cor batch-all-parallel <max_r> [--beam N] [--output dir] [--threads N]
      Same as batch-all but with multi-threading (much faster)
...
Options:
  ...
  --threads N  Number of threads for parallel mode (default: auto)
...
void cmd_info() {
    ...
    // 打印预计算统计
    std::cout << "\nPrecomputed single-active transitions:\n";
    for (int pos = 0; pos < 8; ++pos) {
        size_t total = 0;
        for (int nib = 1; nib < 16; ++nib) {
            total += g_precomp.single_trans[pos][nib].size();
        }
        std::cout << "  pos=" << pos << ": " << total << " total transitions ("
                  << (total / 15) << " avg per nibble)\n";
    }
}
...
// ---------------------------------------------------------------------------
// batch-all-parallel 命令 (并行)
// ---------------------------------------------------------------------------
void cmd_batch_all_parallel(int argc, char* argv[]) {
    if (argc < 3) { print_usage(); return; }
    ...
    Timer total_timer;
    auto all_entries = batch_search_all_parallel(max_rounds, beam_width, num_threads);
    double total_elapsed = total_timer.elapsed();
...
int main(int argc, char* argv[]) {
    ...
    } else if (mode == "batch-all-parallel") {
        cmd_batch_all_parallel(argc, argv);
